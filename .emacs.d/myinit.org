...
;;(load /ssd2/dotfiles/contrib/lisp/ox-confluence.el)
...
** org journal
#+BEGIN_SRC emacs-lisp
;  (when (string= (user-login-name) "vinodh")
...
#+RESULTS:
| lambda | (buffer &optional args) | (switch-to-buffer buffer) | (get-buffer-window buffer 0) |
...
(global-set-key (kbd "<S-f3>") 'init-macro-counter-default)
(global-set-key (kbd "<S-f4>") 'kmacro-insert-counter)
#+END_SRC
